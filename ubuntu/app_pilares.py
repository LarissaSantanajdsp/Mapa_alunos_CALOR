    ...
    if num_alunos > 0:
        ...
        for idx, (aluno, pontos) in enumerate(alunos_com_dados):
            with cols_display[idx % len(cols_display)]:
                ...
                # Cores para cada texto (usar paleta Movimento Calor)
                ...
                # Título
                ax.set_title(f"{aluno}", size=14, fontweight='bold', color=TEXT_COLOR, pad=20)
        ...
        if len(alunos_com_dados) > 0:
            fig, axes = plt.subplots(1, len(alunos_com_dados), figsize=(6*len(alunos_com_dados), 6), subplot_kw=dict(projection='polar'), facecolor=BG_COLOR)
            ...
            for ax, (aluno, pontos) in zip(axes, alunos_com_dados):
                ...
                ax.set_xticks(angles[:-1])
                ax.set_xticklabels(pilares, size=10, fontweight='bold', color=TEXT_COLOR)
                ax.set_ylim(0, 5)
                ax.set_yticks([1, 2, 3, 4, 5])
                ax.set_yticklabels(['1', '2', '3', '4', '5'], size=8, color=TEXT_COLOR, alpha=0.7)
                ax.grid(True, color=TEXT_COLOR, alpha=0.3)
                ax.set_title(f"{aluno}", size=12, fontweight='bold', color=TEXT_COLOR, pad=15)
